let AjaxGET = (url, argument, callback, ender) => {
    if(!url)return;
    ...
    let res = '?';
    if(argument)for(let i in argument)res+=`${i}=${argument[i]}&`;
    res+=ender
    console.log(res)
    xhttp.open('GET',url+res, true);
    xhttp.send();
}
(function ($) {
    'use strict';
    ...
    $('.slider-range-price').each(function () {
        ...
        $(this).slider({
            ...
            slide: function (event, ui) {
                ...
                const ajaxprice = {min:ui.values[0],max:ui.values[1]}
                AjaxGET('/search',{price: JSON.stringify(ajaxprice)},(html)=>{
